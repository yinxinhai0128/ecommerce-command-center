...
const snapshot: DashboardSnapshot = {
  ...
  funnel: [],
  channelRanking: Array.from({ length: 6 }, (_, index) => ({ channel: index % 2 ? '搜索' as const : '信息流' as const, attributedRevenue: 6000 - index * 500, spend: 1000 - index * 100 })),
...
test('构建严格最小化的分析上下文并限制贡献者与告警数量', () => {
  ...
  expect(context).toEqual({
    ...
    topContributors: {
      channels: [
        { label: '信息流', attributedRevenue: 6000, spend: 1000 },
        { label: '搜索', attributedRevenue: 5500, spend: 900 },
        { label: '信息流', attributedRevenue: 5000, spend: 800 },
        { label: '搜索', attributedRevenue: 4500, spend: 700 },
      ],
