...
export function OperationsPage({ snapshot }: OperationsPageProps): JSX.Element {
  const paymentUnavailable = snapshot.capabilities.find(({ key, status }) => key === 'paymentTiming' && status === 'unavailable');
  const fulfillmentUnavailable = snapshot.capabilities.find(({ key, status }) => key === 'fulfillmentOutcomes' && status === 'unavailable');
  return (
    <section className="operations-page" aria-label="经营数据">
      <Panel title="支付方式">{paymentUnavailable ? <p className="panel-empty">{paymentUnavailable.reason}</p> : <ul>{snapshot.payments.byType.map((item) => <li key={item.paymentType}>{item.paymentType}：{currency(item.paymentAmount)}</li>)}</ul>}</Panel>
      ...
      <Panel title="履约状态"><ul>{snapshot.fulfillment.statusDistribution.map((item) => <li key={item.status}>{item.status}：{item.value.toLocaleString('zh-CN')}</li>)}</ul>{fulfillmentUnavailable ? <p className="panel-empty">{fulfillmentUnavailable.reason}</p> : <p>平均送达：{snapshot.fulfillment.averageDeliveryDays.toFixed(1)} 天</p>}</Panel>
