...
export function RealtimeDashboard({ snapshot, alerts, isRunning }: RealtimeDashboardProps): JSX.Element {
  return <div className={`realtime-dashboard ${isRunning ? 'is-running' : 'is-paused'}`}><KpiStrip snapshot={snapshot} /><CommerceOverviewPanels snapshot={snapshot} /><section className="panel sales-pulse"><h2>分钟经营脉冲</h2><SalesPulseChart salesTrend={snapshot.salesTrend} /></section><div className="realtime-side-stack"><RealtimeOrderFeed orders={snapshot.recentOrders} /><AlertRail alerts={alerts} /></div><BreakdownPanels snapshot={snapshot} /></div>;
}
