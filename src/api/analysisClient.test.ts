...
const result: AnalysisResult = {
  ...
  actions: [{
    ...
    rationale: '退款率高于对比期。',
    ownerRole: '商品运营负责人',
    expectedImpact: '降低退款损失',
    validationMetric: '7 天退款率',
  }],
  followUps: ['哪个商品退款最多？'],
  source: 'local',
...
test.each([
  ...
  ['行动包含非法优先级', { ...result, actions: [{ ...result.actions[0], priority: 'urgent' }] }],
  ['行动缺少负责人角色', { ...result, actions: result.actions.map(({ ownerRole: _ownerRole, ...action }) => action) }],
  ['行动缺少预期影响', { ...result, actions: result.actions.map(({ expectedImpact: _expectedImpact, ...action }) => action) }],
  ['行动缺少验证指标', { ...result, actions: result.actions.map(({ validationMetric: _validationMetric, ...action }) => action) }],
  ['行动负责人角色超长', { ...result, actions: [{ ...result.actions[0], ownerRole: 'x'.repeat(1_001) }] }],
