...
import { trustedNumberAllowList } from './analysisContext';
...
function isTrustedNumber(value: number, allowList: ReturnType<typeof trustedNumberAllowList>) {
  return allowList.some((allowed) => Math.abs(value - allowed.value) <= tolerance(allowed.unit));
}

export function hasOnlyTrustedNumbers(analysis: Pick<AnalysisResult, 'signals' | 'causes'>, context: PilotAnalysisContext) {
  const allowList = trustedNumberAllowList(context);
  return analysis.signals.every(({ value }) => isTrustedNumber(value, allowList))
    && analysis.causes.every(({ contribution }) => isTrustedNumber(contribution, allowList));
}
...
export async function requestPilotDeepSeekAnalysis(options: DeepSeekOptions): Promise<PilotDeepSeekOutcome> {
  ...
  const messages = [
    {
      role: 'system',
      content: `只返回 JSON 对象，必须遵循此 JSON schema：${modelSchemaDescription}。signals.value 与 causes.contribution 只能复制可信上下文中的数值。`,
    },
    {
      role: 'user',
